...
export function RolesPage() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Roles</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">Define what team members are allowed to do.</p>
        </div>
        <Button onClick={() => setCreateOpen(true)}>
          <Plus size={16} /> New role
        </Button>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {loading ? (
          ...
          <div className="col-span-full flex flex-col items-center gap-2 py-12 text-center">
            ...
            <p className="text-sm font-medium text-slate-700 dark:text-slate-300">No roles yet</p>
            <p className="max-w-xs text-xs text-slate-400 dark:text-slate-500">Create your first role to start inviting team members.</p>
            <Button onClick={() => setCreateOpen(true)} className="mt-2">
              <Plus size={14} /> New role
            </Button>
          </div>
          ...
          roles.map((role) => (
            <div key={role._id} className="rounded-lg border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Shield size={16} className="text-brand-primary" />
                  <h3 className="font-medium text-slate-900 dark:text-slate-100">{role.role_name}</h3>
                </div>
                <div className="flex gap-1">
                  <button
                    onClick={() => openEdit(role)}
              ...
              <ul className="mt-3 space-y-1">
                {role.permissions.map((p) => (
                  <li key={p} className="text-xs text-slate-500 dark:text-slate-400">
                    {ALL_PERMISSIONS.find((ap) => ap.value === p)?.label || p}
                  </li>
                ))}
              </ul>
      ...
      {deleteResult && (
        <div className="fixed bottom-4 right-4 rounded border border-green-200 bg-green-50 px-4 py-2 text-sm text-green-700 shadow-lg dark:border-green-900/60 dark:bg-green-950/40 dark:text-green-400">
          {deleteResult}
        </div>
      )}
