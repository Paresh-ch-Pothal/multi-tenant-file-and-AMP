...
export function UsersPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      {roles.length === 0 && !loading && (
        <div className="rounded border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-700 dark:border-amber-900/60 dark:bg-amber-950/40 dark:text-amber-400">
          Create a role first before inviting team members.
        </div>
      )}
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              users.map((user) => (
                <tr key={user._id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-800/50">
                  <td className="px-4 py-2.5 text-slate-900 dark:text-slate-100">{user.email}</td>
                  <td className="px-4 py-2.5">
                    {user.role_id ? (
                      <span className="text-slate-500 dark:text-slate-400">{roleName(user.role_id)}</span>
                    ) : (
                      <Badge variant="warning">No role assigned</Badge>
                    )}
                  </td>
                  <td className="px-4 py-2.5"><Badge variant={statusVariant(user.status)}>{user.status}</Badge></td>
                  <td className="px-4 py-2.5 text-slate-500 dark:text-slate-400">{formatDate(user.created_at)}</td>
                  <td className="px-4 py-2.5">
                    <div className="flex gap-1">
                      <button
                        onClick={() => openEdit(user)}
