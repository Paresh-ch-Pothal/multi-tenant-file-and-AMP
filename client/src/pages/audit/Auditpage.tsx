  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">A record of every change made in your organization.</p>
      </div>

      {/* filters */}
      <div className="grid gap-3 rounded-lg border border-slate-200 bg-white p-4 sm:grid-cols-4 dark:border-slate-800 dark:bg-slate-900">
        <Select
          label="Action"
          value={actionFilter}
          onChange={(e) => { setActionFilter(e.target.value); setPage(1); }}
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              logs.map((entry) => (
                <tr key={entry._id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-800/50">
                  <td className="px-4 py-2.5 font-medium text-slate-900 dark:text-slate-100">{actionLabel(entry.action)}</td>
                  <td className="px-4 py-2.5 text-slate-600 dark:text-slate-400">{actorLabel(entry)}</td>
                  <td className="px-4 py-2.5">
                    <Badge variant={entry.status === 'success' ? 'success' : 'danger'}>{entry.status}</Badge>
                  </td>
                  <td className="px-4 py-2.5 font-mono text-xs text-slate-400 dark:text-slate-500">{entry.ip_address || '—'}</td>
                  <td className="px-4 py-2.5 text-slate-500 dark:text-slate-400">{new Date(entry.created_at).toLocaleString()}</td>
                </tr>
              ))
            )}
          </tbody>
