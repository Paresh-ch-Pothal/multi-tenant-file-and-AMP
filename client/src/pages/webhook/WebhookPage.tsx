...
export function WebhooksPage() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Webhooks</h1>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              webhooks.map((wh) => (
                <tr key={wh._id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-800/50">
                  <td className="px-4 py-2.5">
                    <div className="flex items-center gap-2 text-slate-900 dark:text-slate-100">
                      <WebhookIcon size={14} className="shrink-0 text-slate-400 dark:text-slate-500" />
                      <span className="truncate max-w-xs">{wh.url}</span>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-2.5 text-slate-500 dark:text-slate-400">{formatDate(wh.created_at)}</td>
                  <td className="px-4 py-2.5">
                    <button
                      onClick={() => setDeleteTarget(wh)}
